{% embed "snipplets/modal.tpl" with{modal_id: 'home-modal', modal_class: 'home-promotional modal-bottom-sheet modal-shadow', modal_position: 'bottom', modal_transition: 'slide', modal_header_title: true, modal_footer: false, modal_width: 'docked-md modal-docked-small modal-docked-md-right top-line left-line-md', modal_body_class: 'p-0' } %}
    ...
    {% block modal_body %}
        ...
        {% if "home_popup_image.jpg" | has_custom_image %}
            <div class="{% if settings.home_popup_txt or ((settings.home_popup_btn and settings.home_popup_url) and not settings.home_news_box) %}bottom-line{% endif %}">
        ...
        {% if settings.home_news_box %}
            <div id="news-popup-form-container" class="newsletter">
                <form id="news-popup-form" method="post" action="/winnie-pooh" class="js-news-form" data-store="newsletter-form-popup">
                    ...
                    <div class="newsletter-form input-append row no-gutters overflow-none">
                        <div class="col">
                            {% embed "snipplets/forms/form-input.tpl" with{input_for: 'email', type_email: true, input_name: 'email', input_id: 'email', input_placeholder: 'Ingresá tu email...' | translate, input_custom_class: "js-mandatory-field", input_aria_label: 'Email' | translate } %}
                            {% endembed %}
                        </div>
                        <div class="col-auto">
                            ...
                            <input type="submit" name="contact" class="js-news-popup-submit btn btn-big btn-primary" value="{{ "Enviar" | translate }}" />
        ...
        {% elseif settings.home_popup_btn and settings.home_popup_url %}
            <div class="{% if settings.home_popup_txt %}px-3 pb-3{% else %}p-3{% endif %}">
                <a href="{{ settings.home_popup_url }}" class="btn btn-primary d-inline-block btn-medium">{{ settings.home_popup_btn }}</a>
            </div>
        {% endif %}
