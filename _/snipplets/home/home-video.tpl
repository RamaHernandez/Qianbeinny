{% if settings.video_embed %}
    <section class="js-section-video section-video-home bottom-line" data-store="home-video" data-transition="fade-in" {% if settings.head_transparent %}data-header-type="transparent-on-section"{% endif %}>
        ...
        {% if has_video_text %}
            <div class="container-fluid p-0 mb-neg-1">
                <div class="row no-gutters">
                    <div class="col-md-6">
                        <div class="home-video-text top-line right-line-md">
                            {% if settings.video_title or settings.video_subtitle %}
                                <div class="p-3">
                                    {% if settings.video_subtitle %}
                                        <div class="mb-3">{{ settings.video_subtitle }}</div>
                                    {% endif %}
                                    {% if settings.video_title %}
                                        <h2 class="h1-huge mb-1">{{ settings.video_title }}</h2>
                                    {% endif %}
                                </div>
                            {% endif %}
                            {% set has_video_button = settings.video_button and settings.video_button_url  %}
                            {% if has_video_button or settings.video_text %}
                                <div class="row no-gutters justify-content-end{% if settings.video_title or settings.video_subtitle %} top-line{% endif %}">
                                    {% if settings.video_text %}
                                        <div class="col p-3">
                                            <p class="mb-0">{{ settings.video_text }}</p>
                                        </div>
                                    {% endif %}
                                    {% if has_video_button %}
                                        <div class="col-auto col-btn p-3 left-line">
                                            <a href="{{ settings.video_button_url }}" class="btn btn-link d-inline-block">{{ settings.video_button }}</a>
                                        </div>
                                    {% endif %}
                                </div>
                            {% endif %}
