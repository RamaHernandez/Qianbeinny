...
<div class="form-group {{ input_group_custom_class }}">
    {% if input_label_text %}
        <label {% if input_label_id %}id="{{ input_label_id }}"{% endif %} class="form-label d-block px-3 py-2 {{ input_label_custom_class }}" {% if input_for %}for="{{ input_name }}"{% endif %}>{{ input_label_text }}</label>
    {% endif %}
        ...
        />
            ...
            <a aria-label="{{ 'Ver contraseña' | translate }}" class="js-password-view btn form-toggle-eye">
                <span class="js-eye-open" style="display: none;">
                    {% include "snipplets/svg/eye.tpl" with {svg_custom_class: "icon-inline svg-icon-text"} %}
                </span>
                <span class="js-eye-closed">
                    {% include "snipplets/svg/eye-closed.tpl" with {svg_custom_class: "icon-inline svg-icon-text"} %}
                </span>
            </a>                        
            {% endif %}
    {% endif %}
