...
<div class="form-group {{ select_group_custom_class }}">
    {% if select_label %}
        <label {% if select_label_id%}id="{{ select_label_id }}"{% endif %} class="form-label d-block px-3 py-2 {{ select_label_custom_class }}" {% if select_for %}for="{{ select_for }}"{% endif %}>{{ select_label_name }}</label>
    {% endif %}
    <select 
    ...
    <div class="form-select-icon">
        ...
        {% else %}
            {% set svg_icon = 'chevron-down' %}
        {% endif %}
        {% include "snipplets/svg/" ~ svg_icon ~ ".tpl" with {svg_custom_class: "icon-inline icon-w-14"} %}
    </div>
</div>
