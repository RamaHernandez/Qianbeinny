{% if options %}
    ...
    {% if store.show_shipping_emergency_message %}
        <div class="alert alert-warning bottom-line top-no-line">{{ store.shipping_emergency_message }}</div> 
    {% endif %}

    <div class="{% if cart.items_count > 0 and not cart.free_shipping.cart_has_free_shipping %}js-product-shipping-label bottom-line{% endif %} font-small p-3" style="display: none;">
    ...
    {% if has_featured_shipping %}
        ...
        <div class="font-small p-2 text-center text-uppercase bottom-line">
            {{ "Envío a domicilio" | translate }}
        </div>

        
        <ul class="radio-button-container list-unstyled bottom-line">
    ...
    {% if store.has_smart_dates and show_time %}
        <div class="font-small p-3">{{"El tiempo de entrega <strong>no considera feriados</strong>." | translate}}</div>
    {% endif %}
{% else %}
