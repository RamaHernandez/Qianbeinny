{% if product_detail %}
	...
	{% if calculator_label %}
	<div class="js-shipping-add-product-label form-label px-3 py-2" {% if hide_free_shipping_minimum %}style="display: none;"{% endif %}>
	{% else %}
	<div class="js-free-shipping-add-product-title position-absolute transition-up w-100 {% if not hide_free_shipping_minimum %}transition-up-active{% endif %} form-label font-small p-3">
	{% endif %}
...
{% else %}
	<div class="js-visible-on-cart-filled" {% if cart.items_count == 0 %}style="display: none;"{% endif %}>
		<div class="js-fulfillment-info js-allows-non-shippable" {% if not cart.has_shippable_products %}style="display: none"{% endif %}>
			...
			<div class="js-ship-free-rest {% if progress_bar %}cart-row{% endif %} pb-1">
