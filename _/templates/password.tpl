...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:fb="http://www.facebook.com/2008/fbml" xmlns:og="http://opengraphprotocol.org/schema/">
    ...
    <body class="body-password {% if customer %}customer-logged-in{% endif %} template-{{ template | replace('.', '-') }}">
        ...
        <div class="col-md-6 offset-md-3 right-line-md left-line-md right-line-md p-0 flex-grow-1">
            <h2 class="p-5 bottom-line m-0 text-center">{{ message }}</h2>
            {% embed "snipplets/forms/form.tpl" with{form_id: 'password-form', submit_text: 'Desbloquear' | translate, submit_custom_class: 'btn-block btn-big' } %}
                {% block form_body %}

                    {% embed "snipplets/forms/form-input.tpl" with{input_for: 'password', type_password: true, input_name: 'password', input_label_text: 'Contraseña de acceso' | translate, input_group_custom_class: "mb-0", input_placeholder: 'ej.: tucontraseña' | translate } %}
                        {% block input_form_alert %}
                            {% if invalid_password == true %}
                                <div class="alert alert-danger bottom-line top-no-line">{{ 'La contraseña es incorrecta.' | translate }}</div>
                            {% endif %}
